/*
    2020.06.02 수정사항
    사용자 히스토리와 기기관리 히스토리가 분리되어 동작되게끔 수정
    -> 기기관리 히스토리에서 전체조회가 가능(기기번호란에 아무값도 없을 시 전체조회)
    -> 기기번호를 입력받아 사용자 없는 기기를 먼저 등록할 수 있음
    -> 사용자 등록과정 중 취소버튼 클릭해도 기기관리 히스토리에 영향 없음
*/
...
function getUserList(obj) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#useToggleBtn").val("편집");
            $("#clearUseBtn").show();
            getMachineList(no, resultUser.no);
        },
        error: function (err) {
            console.log("ajax 통신 실패");
            console.log(err);
...
function clearMachineForm(initData){
    if(initData !== undefined) $("#devNo").val(initData);
    else $("#devNo").val("");
    ...
    if(toggleVal == "등록"){
        ...
        if(confirm("'" + no + "'기기에 대한 히스토리를 편집하시겠습니까?")){
            $.ajax({
                ...
                success: function (result) {
                    if($("#devNo").val() != $("#inputDevNo").val()){
                        // 수정했는데 사용자 정보가 없을 경우

                        // 수정 후 사용자 기기번호에 대한 수정

                        $("#inputDevNo").val($("#devNo").val());
...
// 기기 목록 조회
function getMachineList(no, initData) {
    // 선택된 기기가 없거나 기기번호 입력값이 없을 경우 기기목록 전체조회
    // if(no == ""){
    //     alert("기기를 선택 또는 입력해주세요.");
    //     return;
    // }
...
function machineDetail(obj){
    ...
    $("#devNo").val($(detail[1]).val());
    $("#inputModel").val($(detail[2]).text());
    $("#inputCpu").val($(detail[3]).text());
    $("#inputSSD1").val($(detail[4]).text());
    $("#inputSSD2").val($(detail[5]).text());
    $("#inputSSD3").val($(detail[6]).text());
    $("#inputHDD1").val($(detail[7]).text());
    $("#inputHDD2").val($(detail[8]).text());
    $("#inputHDD3").val($(detail[9]).text());
    $("#inputMem").val($(detail[10]).text());
    $("#inputOS").val($(detail[11]).text());
    $("#inputVGA").val($(detail[12]).text());
    $("#inputUseDate2").val(dateFormatter($(detail[13]).text()));
    $("#inputReturnDate2").val(dateFormatter($(detail[14]).text()));
    $("#inputPosition").val($(detail[15]).text());
    $("#inputStat").val($(detail[16]).text()).prop("selected", true);
    $("#inputPrice").val($(detail[17]).text());
    $("#devRemark").val($(detail[18]).text());
